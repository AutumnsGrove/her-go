...
func TestFormatTrace(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "save_memory shows memory details",
			tool:     "save_memory",
			argsJSON: `{"memory":"likes coffee","category":"preference","importance":5}`,
			result:   "saved user memory ID=42",
			wantSub:  "category=preference",
		},
		{
			name:     "save_memory rejection shows reject emoji",
			tool:     "save_memory",
			argsJSON: `{"memory":"some memory","category":"other","importance":1}`,
