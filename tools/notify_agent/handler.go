...
// a background agent back to the main agent and triggers a follow-up message.
//
// When the memory agent finishes inbox tasks (cleanup, splits), it calls
// notify_agent instead of done. This does three things:
//   1. Writes the result to the inbox (so the main agent can read details)
//   2. Fires an AgentEvent to wake up the main agent
//   3. Ends the memory agent's turn (sets DoneCalled = true)
//
// The main agent then reads the inbox and sends a brief follow-up to the user.
...
// Handle writes results to the inbox and fires an event to wake up the main agent.
func Handle(argsJSON string, ctx *tools.Context) string {
	...
	// Write the result to the inbox so the main agent can read details.
	...
	// Fire the agent event to wake up the main agent. The bot layer
	...
	return fmt.Sprintf("notified main agent: %s", args.Summary)
}
