...
- [x] **Phase 6: Testing & sims** — Complete. Sim runner supports `seed_cards` and structured `{card, content}` seed memories with backward-compatible plain strings. Updated 5 existing sims with card assignments. Created 3 new sims: `card-lifecycle` (end-to-end), `card-scoped-recall` (scoped search), `self-memory-quality` (technique log rejection). All tests green, race-clean.
